        ...
        class adventureplaces:
            #def __init():
             ...
             def fetch(self):
                ...
                da =soup.select(".rating-text")
                soup = BeautifulSoup(r.text, 'html.parser')
                daname =soup.select(".ne-title")
                li1 = []
                for x in xrange(1,10):
                    pass items in daname:
                    name = items.text.strip().encode('utf8')
                    li1.append(name)
#print(li1)
                li2 = []
                for names in da:
                 rev=names.contents[3].text.encode('utf8')
                li2.append(rev)
#print(li1)

file = open('E:/monumentsreviewfile5', 'a')
match = {}
for key in li1:
    for value in li2:
            match[key] =value
    ...
    if destination == 'Delhi':
            ...
            class Monuments:
                ...
                def fetch(self):
                    ...
                    url = "https://www.treebo.com/blog/historical-monuments-in-delhi/"
                    r = requests.get(url)
                    soup = BeautifulSoup(r.text, 'html.parser')
                    da = soup.select(".ez-toc-section")
                    file = open(r'monument-Delhi.txt', 'w+')
                    data = []
                    for names in da:
                        # textda = names.text
